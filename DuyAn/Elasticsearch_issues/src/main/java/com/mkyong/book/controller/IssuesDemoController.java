...
public class IssuesDemoController {
	...
	public ResponseEntity<Iterable<IssuesDemo>> ListAllStaff() {
		Iterable<IssuesDemo> issue = issuesDemoService.findAllIssuesDemo();
		//System.out.println("An Gay"+staff.get(0).getDailyCost());
		if (issue==null) {
			return new ResponseEntity(HttpStatus.NO_CONTENT);
			// You many decide to return HttpStatus.NOT_FOUND
		}
		return new ResponseEntity<Iterable<IssuesDemo>>(issue, HttpStatus.OK);
